# Global Debug Dashboard Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace SnifferDebugDialog with a tabbed global debug dashboard showing Sniffer status, Service status, Event stream, and Thread info in a card-based visual style.

**Architecture:** Single JDialog rewrite. Tab bar at top switches between 4 content panels. Shared 500ms Timer refreshes only the active tab. EventBus listener captures events into a bounded ring buffer for the Events tab.

**Tech Stack:** Java 25, Swing (JDialog, JPanel, BoxLayout, GridBagLayout), FlatLaf theming via ThemeManager, EventBus
...
|------|--------|---------------|
| `src/com/superredrock/usbthief/gui/dailog/SnifferDebugDialog.java` | **Rewrite** | Entire debug dashboard: tab bar, 4 content panels, refresh timer, event capture |
| `src/com/superredrock/usbthief/gui/MainFrame.java` | **Modify** (lines 44, 268-270, 411-416) | Change field type and dialog opening — no logic change needed since constructor signature stays the same |

---

### Task 1: Rewrite SnifferDebugDialog — shell, tab bar, and Sniffers tab

**Files:**
- Rewrite: `src/com/superredrock/usbthief/gui/dailog/SnifferDebugDialog.java`

- [ ] **Step 1: Write the complete SnifferDebugDialog with shell + Sniffers tab**

Replace the entire file content with the following. This implements:
- Window shell (640x480, non-modal, resizable)
- Tab bar with pill-shaped buttons
- Sniffers tab with card-based layout
- 500ms shared refresh timer
- Event capture ring buffer (for later tabs)
...
public class SnifferDebugDialog extends JDialog {
    ...
    private JPanel buildContentArea() {
        ...
        container.add(sniffersPanel, "SNIFFERS");
        container.add(servicesPanel, "SERVICES");
        container.add(eventsPanel, "EVENTS");
        container.add(threadsPanel, "THREADS");
        return container;
    }
    ...
    private JPanel buildSnifferCard(SnifferDebugSnapshot s) {
        ...
        // Header: drive icon + name/serial | badge
    ...
    private JPanel buildServiceCard(Service svc) {
        ...
        // Bottom: tick info
        String tickInfo = svc.getTickInterval() + " " + svc.getTickUnit().toString().toLowerCase();
        JLabel infoLabel = new JLabel("Tick: " + tickInfo);
...
- [ ] **Step 2: Verify MainFrame compatibility**

The existing `SnifferDebugDialog(Frame)` constructor signature is preserved, so MainFrame's `showDebugMonitor()` method (line 411-416) and field declaration (line 44) need no changes. Just verify no other files reference SnifferDebugDialog:

Run: `grep -r "SnifferDebugDialog" src/ --include="*.java" | grep -v "SnifferDebugDialog.java"`

Expected: Only MainFrame.java references found, and they use `new SnifferDebugDialog(this)` which matches our preserved constructor.
...
```bash
git add src/com/superredrock/usbthief/gui/dailog/SnifferDebugDialog.java
git commit -m "feat: rewrite SnifferDebugDialog as global debug dashboard with 4 tabs"
```
