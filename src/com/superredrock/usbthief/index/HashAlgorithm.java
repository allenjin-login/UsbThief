...
public enum HashAlgorithm {
     ...
     */
    public CheckSum compute(Path path, ByteBuffer buffer) throws IOException {
        byte[] hash;
        switch (this) {
            case SHA_256:
            case MD5:
                hash = computeMessageDigest(path, buffer);
                break;
            case CRC_8:
                hash = computeWithChecksum(path, buffer, new CRC8());
                break;
            case CRC_16:
                hash = computeWithChecksum(path, buffer, new CRC16());
                break;
            case CRC_32:
                hash = computeWithChecksum(path, buffer, new CRC32J());
                break;
            case CRC_64:
                hash = computeWithChecksum(path, buffer, new CRC64());
                break;
            default:
    ...
    private byte[] computeMessageDigest(Path path, ByteBuffer buffer) throws IOException {
        try {
            MessageDigest digest = MessageDigest.getInstance(id);
            try (FileChannel ch = FileChannel.open(path, StandardOpenOption.READ)) {
                while (ch.read(buffer) != -1) {
                    if (Thread.interrupted()) throw new IOException("Hash computation interrupted");
                    buffer.flip();
    ...
    private byte[] computeWithChecksum(Path path, ByteBuffer buffer, Checksum checksum) throws IOException {
        try (FileChannel ch = FileChannel.open(path, StandardOpenOption.READ)) {
            while (ch.read(buffer) != -1) {
                if (Thread.interrupted()) throw new IOException("Hash computation interrupted");
                buffer.flip();
